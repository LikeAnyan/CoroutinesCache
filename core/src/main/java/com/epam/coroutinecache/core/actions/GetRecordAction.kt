...
class GetRecordAction(
     ...
     */
    suspend fun <T: Any> getRecord(key: String, entryClass: KClass<T>, useRecordEvenIfExpired: Boolean = false): Record<T>? = scope.async {
        var record = memory.getRecord<T>(key)

        if (record != null) {
            record.setSource(Source.MEMORY)
        } else {
            record = diskCache.getRecord(key, entryClass)
            if (record != null) {
